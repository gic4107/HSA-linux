...
static int hmm_exit(struct notifier_block *nb,
		    ...
		    void *data)
{
	...
	spin_lock(&hmm->lock);
	BUG_ON(!list_empty(&hmm->mirrors));
	spin_unlock(&hmm->lock);
	/* Drop the reference taken by >hmm_release mmu_notifier callback. */
	hmm_unref(hmm);
	return 0;
}
...
static void hmm_notifier_release(struct mmu_notifier *mn, struct mm_struct *mm)
{
	...
	while (mirror) {
		...
		BUG_ON(!mirror);
		spin_unlock(&hmm->lock);
		/* The test is not protected against concurrent update
		 * and is just a best effort to avoid useless call to
		 * >mirror_release.
		 */
 ...
 * the mirror.
 *
 * Caller must hold a reference on the mirror they are passing as argument. So
 * that the structure can not be free while inside this function is running.
 */
void hmm_mirror_unregister(struct hmm_mirror *mirror)
{
	 ...
	 * release concurrently with us and thus the mirror structure is being
	 * destroy concurrently and its fields can no longer be trusted.
	 *
	 * The hmm object is use as synchronization point. If we can still take
	 * a reference on it than we can rely on it otherwise just assume that
	 * there is nothing to wait for as by the time hmm goes away all mirror
	 * activities are done.
	 */
	if (!mirror)
		return;
	...
	/* The test is not protected against concurrent update and is just a
	 * best effort to avoid useless call to >mirror_release.
	 */
	if (mirror->mlist.prev != LIST_POISON2)
		mirror->device->ops->mirror_release(mirror);

	/* Check if the mirror is already removed from the mirror list in which
	 * case there is nothing to do except wait for device list to be set to
	 * empty list meaning no more callback can happen.
	 */
	spin_lock(&hmm->lock);
	if (mirror->mlist.prev != LIST_POISON2) {
		list_del_rcu(&mirror->mlist);
...
static void hmm_mirror_delayed_destroy(struct work_struct *work)
{
	struct hmm_mirror *mirror;
	struct hmm *hmm;

	mirror = container_of(work, struct hmm_mirror, destroy);
	/* Take a reference on hmm so we can wake_up any one waiting. */
	hmm = hmm_ref(mirror->hmm);
	hmm_mirror_unref(mirror);
	if (!hmm) {
		BUG();
...
static void hmm_mirror_handle_error(struct hmm_mirror *mirror)
{
	...
	spin_lock(&hmm->lock);
	if (mirror->mlist.prev == LIST_POISON2) {
		/* Another thread is already destroying this mirror. */
		spin_unlock(&hmm->lock);
		return;
	}
	list_del_rcu(&mirror->mlist);
	...
	/* Because this is call from mmu_notifier callback it can not directly
	 * unreference the hmm structure as it could potentialy deadlock with
	 * mmu_notifier_unregister. We have to delay this to a workqueue use
	 * the system one.
	 */
	INIT_WORK(&mirror->destroy, hmm_mirror_delayed_destroy);
	schedule_work(&mirror->destroy);
}

static void hmm_mirror_destroy_delayed(struct kref *kref)
{
	struct hmm_mirror *mirror;

	mirror = container_of(kref, struct hmm_mirror, kref);
	/* We can reuse same code as handle_error for now. */
	hmm_mirror_handle_error(mirror);
}

static struct hmm_mirror *hmm_mirror_unref_delayed(struct hmm_mirror *mirror)
{
	if (mirror)
		kref_put(&mirror->kref, hmm_mirror_destroy_delayed);
	return NULL;
}
...
{
	mutex_lock(&device->mutex);
	while (!list_empty(&device->mirrors)) {
		struct hmm_mirror *mirror[16], *tmp;
		unsigned count = 0, i;

		list_for_each_entry(tmp, &device->mirrors, dlist) {
			/* Try to take a reference on mirror and ignore those
			 * for which the hmm_mirror_destroy is being call. The
			 * extra reference enforce that the mirror is not free
			 * while we call hmm_mirror_unregister. Batch at most
			 * 16 unregister together.
			 */
			mirror[count] = hmm_mirror_ref(tmp);
			if (mirror[count]) {
				hmm_mirror_unregister(mirror[count]);
				count++;
				if (count >= 16)
					break;
			}
		...
		mutex_unlock(&device->mutex);
		/* Drop the extra reference we took on all the mirror that were
		 * just unregistered.
		 */
		for (i = 0; i < count; ++i) {
			hmm_mirror_unref(mirror[i]);
		}
		mutex_lock(&device->mutex);
	}
	mutex_unlock(&device->mutex);
				 ...
				 struct hmm_fence *fence)
{
	...
	mirror = hmm_mirror_unref_delayed(mirror);
...
static struct notifier_block hmm_mmput_nb = {
	.notifier_call		= hmm_exit,
	.priority		= 3,
};
...
static int __init hmm_module_init(void)
{
	...
	err = mmput_register_notifier(&hmm_mmput_nb);
	if (err)
		return err;
	return 0;
}
...
{
	mmput_unregister_notifier(&hmm_mmput_nb);
}
